...
def tests_cat_vs_num(df: pd.DataFrame, cat: str, num: str) -> Dict[str, float]:
	...
	if len(groups) == 2:
		...
		welch_stat, welch_p = stats.ttest_ind_from_stats(
			mean1=np.mean(groups[0]), std1=np.std(groups[0], ddof=1), nobs1=len(groups[0]),
			mean2=np.mean(np.concatenate(groups[1:])), std2=np.std(np.concatenate(groups[1:]), ddof=1), nobs2=len(np.concatenate(groups[1:])),
			equal_var=False
		)
		kw_stat, kw_p = stats.kruskal(*groups)
		res.update({
			"anova_stat": float(anova_stat), "anova_p": float(anova_p),
			"welch_stat": float(welch_stat), "welch_p": float(welch_p),
