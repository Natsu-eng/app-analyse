...
class EvaluationMetrics:
    ...
    def calculate_classification_metrics(self, 
        ...
        try:
            ...
            metrics['accuracy'] = self.safe_metric_calculation(
            ...
            try:
                ...
                logger.warning(f"⚠️ Rapport classification échoué: {e}")
    ...
    def calculate_regression_metrics(self, y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            if (np.all(y_true > 0) and np.all(y_pred > 0) and 
                ...
                metrics['msle'] = None
                self.warning_messages.append("MSLE non calculé: valeurs négatives ou infinies détectées")
            ...
            if metrics['mae'] is not None and metrics['rmse'] is not None:
                try:
                    ...
                    logger.warning(f"⚠️ Statistiques erreurs échouées: {e}")
            ...
            try:
                ...
                logger.warning(f"⚠️ Calcul amélioration baseline échoué: {e}")
