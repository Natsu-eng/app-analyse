...
def train_models(
    ...
    # Limitation des caractéristiques avec constante
    ...
    # MODIFICATION: Utilisation de la constante pour la taille des échantillons
    ...
    for i, model_name in enumerate(model_names, 1):
        ...
        if training_result["success"] and training_result["model"] is not None:
            try:
                ...
                # Préparer les échantillons pour visualisations avec la constante
                if task_type == 'clustering':
                    # Pour clustering: échantillon de X et prédictions
                    ...
                    # Pour supervised learning: échantillons de train/test
                    sample_size_test = min(MAX_SAMPLE_SIZE, len(X_test))
                    sample_size_train = min(MAX_SAMPLE_SIZE, len(X_train))
                    
                    X_test_vis = X_test[:sample_size_test].copy()
                    y_test_vis = y_test[:sample_size_test].copy()
                    X_train_vis = X_train[:sample_size_train].copy()
                    y_train_vis = y_train[:sample_size_train].copy()
                    X_sample = X_test[:sample_size_test].copy()
                    labels_sample = None
